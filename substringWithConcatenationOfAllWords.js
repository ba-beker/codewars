...
var findSubstring = function (s, words) {
  ...
  // Populate the word map with the count of each word
  ...
  for (let i = 0; i <= s.length - totalLength; i++) {
    ...
    for (j = 0; j < totalLength; j += wordLength) {
      ...
      // Compare the word occurrences in the substring and wordMap
      if (substringMap[word] > wordMap[word]) {
        break;
      }
    }

    // If all words are present in the correct count, add the starting index to the result
    if (j === totalLength) {
      result.push(i);
    }
  }
